...
<template>
            ...
            <div class="mx-auto">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-4">
                        <button v-if="isEditMode" @click="handleSave" class="px-6 py-2 bg-green-600 text-white font-semibold rounded-md shadow-sm hover:bg-green-700">
                          Salvar e Visualizar
                        </button>
                        <button v-else @click="isEditMode = true" class="px-6 py-2 bg-blue-600 text-white font-semibold rounded-md shadow-sm hover:bg-blue-700">
                          Editar Molde
                        </button>
                    </div>
                </div>
    ...
    <div class="max-w-6xl mx-auto mt-8 bg-white p-6 rounded shadow">
      ...
      <!-- <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 class="text-3xl font-bold text-gray-900">Meu Cronograma Semanal</h1>
          <p class="text-gray-600 mt-1">{{ isEditMode ? 'Arraste as matérias para montar seu plano.' : 'Marque suas tarefas concluídas de hoje.' }}</p>
        </div>
        <div class="flex items-center gap-4">
          <button v-if="isEditMode" @click="handleSave" class="px-6 py-2 bg-green-600 text-white font-semibold rounded-md shadow-sm hover:bg-green-700">
    ...
    <div class="bg-white p-6 rounded-lg shadow sticky top-6">
                    ...
                    <div class="bg-gray-100 p-3 rounded-md cursor-grab text-sm font-medium text-gray-700 hover:bg-gray-200">
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="flex justify-end items-center gap-1">
                                <span 
                                    v-for="initial in subjectUsage[subject.name]" 
                                    :key="initial"
        ...
        <div class="lg:col-span-3">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div v-for="day in scheduleStore.weeklyPlan" :key="day.day" class="bg-white rounded-lg shadow p-5 flex flex-col">
              <h3 class="text-xl font-bold text-gray-800 border-b pb-3 mb-4">{{ day.day }}</h3>
              <draggable v-model="day.subjects" item-key="id" tag="div" group="subjects" class="flex-grow min-h-[10rem] bg-gray-50 rounded-lg p-2 space-y-2" :data-day="day.day" @add="onAddSubject">
                <template #item="{ element: subject }">
                  <div class="flex items-center justify-between bg-blue-100 text-blue-800 text-sm font-medium px-3 py-2 rounded-md cursor-grab">
                    <span>{{ subject.name }}</span>
                    <button @click="removeSubjectFromDay(day.day, subject.id)" class="ml-2 text-blue-800 hover:bg-blue-800/20 rounded-full w-5 h-5 flex items-center justify-center" title="Remover">&times;</button>
                  </div>
                </template>
              </draggable>
            </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="flex-grow min-h-[10rem] space-y-2">
            <p v-if="day.subjects.length === 0" class="text-sm text-gray-400 text-center py-4">Nenhuma matéria planejada.</p>
            <div v-else v-for="subject in day.subjects" :key="subject.id">
              <label 
                class="flex items-center p-3 rounded-md transition-all duration-200"
                :class="[
                  scheduleStore.isCompleted(subject.id) ? 'bg-green-100' : 'bg-gray-100',
                  isToday(day.day) ? 'cursor-pointer hover:bg-gray-200' : 'cursor-not-allowed'
                ]"
              >
                <input 
                  type="checkbox"
                  class="h-5 w-5 rounded border-gray-400 text-green-600 focus:ring-green-500 disabled:cursor-not-allowed"
                  :checked="scheduleStore.isCompleted(subject.id)"
                  :disabled="!isToday(day.day)"
