...
function startTimer() {
    if (!isRunning.value) {
        ...
        timerStore.start(); // Inicia timer no Store
...
function pauseTimer() {
    if (isRunning.value) {
        ...
        timerStore.pauses.push(timerStore.elapsedTime);
    }
}
...
<template>
    <div class="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center">
        ...
        <div class="text-center text-white">
            ...
            <div class="space-x-4">
                <button @click="startTimer" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded"
                    :disabled="isRunning">
                    Iniciar
                </button>
                <button @click="pauseTimer" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded"
                    :disabled="!isRunning">
                    Pausar
                </button>
